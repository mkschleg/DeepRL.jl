...
function build_ann_from_config(rng::Random.AbstractRNG, in, config; kwargs...)
    # The config will be a list.
    layers = []
    for cl in config
        l = build_layer_from_config(rng, in, cl; kwargs...)
        push!(layers, l)
        in = outputsize(l, in)
    end
...
function build_layer_from_config(rng, in, config; kwargs...)
...
function build_layer(::Val{:Flux}, ::Val{:Dense}, in, config; rng=Random.default_rng(), init=get_init_func(config, rng), kwargs...)
    ...
    out = if config["out"] isa String
        get_kwarg_config(config["out"], kwargs)
    else
        config["out"]
    end
