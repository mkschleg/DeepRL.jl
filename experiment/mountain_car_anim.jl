...
module MountainCarAnimExperiment
...
function get_heatmap(agent::DQNAgent, data_range; kwargs...)
    ...
    # y = vcat([Flux.data(agent.model(hcat(data[i,:]))) for i in 1:size(data)[1]])
    return heatmap(
...
mutable struct HeatmapAnimation
    ...
    counter::Int64
    num_frames::Int64
    HeatmapAnimation(counter_init) = new(counter_init, 0, 0)
end

# HeatmapAnimation(counter_init::Int64) = HeatmapAnimation(counter_init)

function plot_heatmap!(hmanim::HeatmapAnimation, agent::DQNAgent, data_range)
    if hmanim.counter <= 1
...
function episode!(env, agent, rng, max_steps, anim, data_range)
    ...
    while !terminal
        ...
        total_rew += rew
        steps += 1
        plot_heatmap!(anim, agent, data_range)
    end
